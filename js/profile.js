...
async function loadUserProfile(userId) {
  if (!window.supabaseClient) {
    setTimeout(() => loadUserProfile(userId), 100);
    return;
  }
  ...
  try {
    ...
    const { data: pastes } = await window.supabaseClient
      .from('pastes')
    ...
    if (container) {
      container.innerHTML = '<div class="empty-state">Profile not found</div>';
    }
  }
}
...
function displayProfile(profile, pastes) {
  const isRich = profile.email === window.RICH_USER_EMAIL;
  const isOwnProfile = window.currentUser && window.currentUser.id === profile.id;
  const container = document.getElementById('profileViewContainer');
  if (!container) return;
...
async function handleAvatarUpload(event) {
  ...
  if (!file || !window.currentUser || !window.supabaseClient) return;

  const isRich = window.currentUser.email === window.RICH_USER_EMAIL;
