...
async function loadAllPastes() {
  ...
  try {
    ...
    tbody.innerHTML = data.map(paste => {
      const isRich = paste.profiles?.email === window.RICH_USER_EMAIL;
...
async function loadPasteByTitle(title) {
  if (!window.supabaseClient) {
    setTimeout(() => loadPasteByTitle(title), 100);
    return;
  }
  ...
  try {
    ...
    const { data: paste, error } = await window.supabaseClient
      .from('pastes')
    ...
    // Increment views
    try {
      await window.supabaseClient.rpc('increment_paste_views', { paste_id: paste.id });
    } catch (e) {
      console.warn('Could not increment views:', e);
    }

    // Load comments
    const comments = await window.loadComments(paste.id);
      ...
      <div class="comments-section">
        ...
        <div id="commentsList" style="margin-top: 20px;">
          ${comments && comments.length > 0 ? comments.map(comment => {
            const isCommentRich = comment.profiles?.email === window.RICH_USER_EMAIL;
            return `
              <div class="comment">
  ...
  try {
    const { data, error } = await window.supabaseClient
      ...
      .insert([{
        ...
        username: window.currentProfile?.username || 'Anonymous',
        views: 0,
...
window.loadAllPastes = loadAllPastes;
window.loadPasteByTitle = loadPasteByTitle;
window.uploadPaste = uploadPaste;
window.searchPastes = searchPastes;